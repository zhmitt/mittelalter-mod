...
# ---------------------------------------------------------------------------
# macOS BSD mktemp does not honor a suffix after XXXXXX; create the temp file
# without suffix and execute via `python3 <file>` (no .py extension required).
...
for m in re.finditer(r'`([^`]+)`', text):
    ...
    has_slash = '/' in token
    has_ext = bool(re.search(r'\.(ts|tsx|js|jsx|json|md|sh|mjs|cjs|py|yaml|yml|css|html|rules|toml)$', token))
...
# ---------------------------------------------------------------------------
# Search helpers — default scope is repo root (template structure has no
# single canonical source dir; rg honours .gitignore)
# ---------------------------------------------------------------------------
rg_found_fixed() {
  local pattern="$1"
  local scope="${2:-.}"
    ...
    case "$ctype" in
      ...
      file)
        ...
        if [[ -f "$repo_root/$fpath" ]]; then
          ...
          if find \
              "$repo_root/src" "$repo_root/scripts" "$repo_root/tests" \
              "$repo_root/workflow" "$repo_root/docs" "$repo_root/openspec" \
              "$repo_root/.github" "$repo_root/.git-hooks" \
              "$repo_root/.claude" "$repo_root/.codex" "$repo_root/.gemini" \
              -name "$fname" -type f 2>/dev/null | grep -q .; then
            result_statuses+=("pass")
